...
This is a high-speed tool designed to efficiently download Cambridge International A-Level past papers from `pastpapers.papacambridge.com`.

It features **both** a sleek **Graphical Web UI** (highly recommended for beginners) and a powerful **Command-Line Interface (CLI)** for power users.

The script uses a parallel brute-force approach to acquire files. It has two modes of operation:
...
* **Beautiful Web UI**: An interactive, browser-based dashboard with live progress bars, speed metrics (files/sec), and activity logs.
...
To use the tool, you need both the Python script and the HTML file.

1. Download both `cie_downloader.py` and `index.html`.
2. Place them together inside the **same folder**.
...
Downloaded files will be automatically saved into a new `CIE_OUT` folder generated in this same directory.

---

## 5. Usage: Graphical Web UI (Recommended)

If you have never used a command-line tool before, or simply prefer a visual interface, use the Web UI! It gives you a clean dashboard to select your settings, view live download speeds, and cancel jobs mid-way if needed.

1. Open your terminal (Mac/Linux) or Command Prompt (Windows).
2. Use the `cd` command to navigate to the folder where you saved the files.
3. Run this simple command:
```bash
python cie_downloader.py --ui

```


4. The script will start a lightweight local server and **automatically open the dashboard in your default web browser**.
5. Fill in your syllabus details, click **⚡ Fetch Papers**, and watch the progress!

---

## 6. Usage: Command-Line Interface (CLI)

If you prefer to bypass the browser, you can run the tool purely via the terminal.
...
| --- | --- | --- | --- | --- |
| `--ui` |  | No | off | Launches the graphical web dashboard in your browser. |
...
| `-fs`, `--file_structure` | `<choice>` | No | `year_month_paper` | Output directory structure. See Section 7. |
...
### Recommended Settings (CLI)

For the best balance of speed and avoiding rate limiting:

```bash
python cie_downloader.py -s 9709 --start_year 2020 --end_year 2023 -j 8 -pj 16 --simul
...
* The website may be temporarily down or rate-limiting you — try reducing your Worker counts.

**Downloads are slow or stalling:**

* You may be getting rate-limited. Lower your worker counts: try 4 Download Workers and 8 Probe workers.
* Turn off `Simultaneous mode` and let the two phases run separately to see if probing alone completes cleanly.

**`ImportError: No module named 'requests'` or `'tqdm'`:**

* Run `pip install requests tqdm` in your terminal and try again.
